...
public class LessonSubmissionService {
              ...
              .filter(
                  attachment ->
                      attachment.getSubmissionAttachmentId() != null
                          && deleteAttachmentIds.contains(attachment.getSubmissionAttachmentId()))
              .toList();
    ...
    long totalRemainingSize =
        existingAttachments.stream()
            .filter(
                attachment ->
                    attachment.getSubmissionAttachmentId() == null
                        || !deletedAttachmentIds.contains(attachment.getSubmissionAttachmentId()))
  ...
  public LessonSubmissionResult getSubmission(Long lessonEnrollmentId) {
    LessonSubmissionModel submission =
        lessonSubmissionRepository
            ...
            .orElseThrow(
                () ->
                    new NotFoundException("lesson.submission.not.found", "Không tìm thấy bài nộp"));
    ...
    var comments =
        submissionCommentRepository.findByLessonSubmissionIdOrderByCommentAtDesc(
            submission.getLessonSubmissionId());
    Long learnerUserId =
        lessonEnrollmentRepository.findUserIdByLessonEnrollmentId(lessonEnrollmentId).orElse(null);
  ...
  @Transactional(readOnly = true)
  public List<LessonSubmissionResult> getSubmissionsByCourseEnrollment(
    ...
    return courseLessonRepository.findLessonIdsByCourseId(courseEnrollment.getCourseId()).stream()
        .map(
            lessonId ->
                lessonEnrollmentRepository.findLessonEnrollmentId(courseEnrollmentId, lessonId))
