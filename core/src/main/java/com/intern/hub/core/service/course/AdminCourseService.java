...
public class AdminCourseService {
  ...
  public void updateCourse(
    ...
    if (lessonIds != null) {
      ...
      List<Long> addedLessonIds =
          normalizedLessonIds.stream()
  ...
  private void validateRequiredAssignments(List<Long> evaluatorUserIds, List<Long> positionIds) {
    if (distinctOrdered(evaluatorUserIds).isEmpty()) {
      throw new BadRequestException(
          "course.evaluators.required", "Khóa học phải có ít nhất 1 người đánh giá");
    }
    if (distinctOrdered(positionIds).isEmpty()) {
      throw new BadRequestException(
          "course.positions.required", "Khóa học phải có ít nhất 1 vị trí học viên");
    }
  }
