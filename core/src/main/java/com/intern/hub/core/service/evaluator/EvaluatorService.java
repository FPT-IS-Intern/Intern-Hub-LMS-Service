...
public class EvaluatorService {
  ...
  public Page<EvaluatorCourseOverviewModel> getCourseOverviews(
    ...
    if (onlyEvaluable) {
      ...
      return courseEvaluatorRepository.findCourseOverviewsByEvaluatorUserId(
          evaluatorUserId, pageable);
    }
  ...
  public java.util.List<EvaluatorSubmissionOverviewModel> getCourseSubmissions(
    ...
    var evaluatorUserIds =
        courseEvaluatorAssignmentRepository.findEvaluatorUserIdsByCourseId(courseId);
    if (evaluatorUserIds.stream().noneMatch(evaluatorUserId::equals)) {
      throw new ForbiddenException(
          "evaluator.course.forbidden", "Ban khong duoc phan cong danh gia khoa hoc nay");
    }

    var submissions = lessonSubmissionRepository.findByCourseId(courseId);
    var users =
        userDirectoryRepository.findByIds(
            submissions.stream()
                .map(EvaluatorSubmissionOverviewModel::getUserId)
                .distinct()
                .toList());
    var userMap =
        users.stream()
            .collect(java.util.stream.Collectors.toMap(user -> user.getUserId(), user -> user));
            ...
            item -> {
              ...
              item.setLearnerNote(
                  comments.stream()
                      .filter(
                          comment ->
                              java.util.Objects.equals(comment.getUserId(), item.getUserId()))
  ...
  public void commentSubmission(Long lessonSubmissionId, Long evaluatorUserId, String comment) {
    if (comment == null || comment.isBlank()) {
      throw new BadRequestException(
          "submission.comment.invalid", "Noi dung nhan xet khong duoc de trong");
    }

    lessonSubmissionRepository
        .findById(lessonSubmissionId)
        .orElseThrow(
            () -> new NotFoundException("lesson.submission.not.found", "Khong tim thay bai nop"));
